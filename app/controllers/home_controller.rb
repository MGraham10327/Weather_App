class HomeController < ApplicationController
  def index
    ...
    if params[:city] != nil
        params[:city].gsub! " ", "_"
      response = HTTParty.get("http://api.wunderground.com/api/00b1ce09b19043cb/geolookup/conditions/q/#{params[:state]}/#{params[:city]}.json")
    else
      response = HTTParty.get("http://api.wunderground.com/api/00b1ce09b19043cb/geolookup/conditions/q/NC/Charlotte.json")
    end
